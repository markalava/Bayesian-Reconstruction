...
This pacakge is available from (https://github.com/markalava/Bayesian-Reconstruction/popReconstructWrappers). It contains wrapper functions and code for two-sex population reconstruction described in @wheldon_bayesian_2015. 


# Set-Up {#sec:Set-Up} 

```{r getting-files, echo = FALSE}
## Install the package
library("devtools")
...
- sex ratios at birth
and life table 'separation factors' to separate cohort deaths into period deaths. 
...
A larger sample was prepared earlier:

```{r recon-larger-run}
## Not run
## (load("extras_large/ThaiMcmc.RData"))
```
...
```{r prior-sample-larger}
## Not run
## (load("extras_large/Thailand_Example_Prior.RData"))
```
...
```{r plot-quantiles-tfr, fig.show = TRUE, fig.cap = "Prior and posterior 95% quantiles, and medians, for total fertility rate, Thailand, 1960--2000."}
...
```{r plot-quantiles-e0, fig.show = TRUE, fig.cap = "Prior and posterior 95% quantiles, and medians, for life expectancy at birth, Thailand, 1960--2000."}
ggplot(subset(qrecon, param == "e0"), aes(x = year, y = param.50pctl)) +
    geom_line(aes(col = legend), size = 1.1) + geom_point(aes(col = legend)) +
    facet_wrap(facets = "sex", ncol = 1, nrow = 2) +
    geom_ribbon(aes(ymin = param.2.5pctl, ymax = param.97.5pctl, fill = legend), alpha = 0.2) 
``` 
