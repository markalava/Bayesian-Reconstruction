...
###
### (From 20120727)
### - Add functionality to save chains mid-run.
###
################################################################################
...
popRecon.logit.aug07 <- function(p) log(p / (1 - p))

popRecon.invlogit.aug07 <- function(x)
{
    if(any(is.infinite(exp(x)))) {
        y <- x
        y[is.infinite(exp(x))] <- 1
        y[!is.infinite(exp(x))] <-
            popRecon.invlogit.aug07(y[!is.infinite(exp(x))])
        return(y)
    }
...
popRecon.rinvGamma.aug07 <- function(n, shape, scale)
{
...
popRecon.dinvGamma.aug07 <- function(x, shape, scale, log = FALSE)
{
    if(log) d <-
...
popRecon.makeColNames.aug07 <- function(m)
{
             ...
             )
{
    ...
    log.sigmasq.f.prior <-
        popRecon.dinvGamma.aug07(sigmasq.f, alpha.f, beta.f, log = TRUE)
    log.sigmasq.s.prior <-
        popRecon.dinvGamma.aug07(sigmasq.s, alpha.s, beta.s, log = TRUE)
    log.sigmasq.g.prior <-
        popRecon.dinvGamma.aug07(sigmasq.g, alpha.g, beta.g, log = TRUE)
    log.sigmasq.n.prior <-
       popRecon.dinvGamma.aug07(sigmasq.n, alpha.n, beta.n, log = TRUE)
    log.sigmasq.srb.prior <-
       popRecon.dinvGamma.aug07(sigmasq.srb, alpha.srb, beta.srb, log = TRUE)
             ...
             )
{
    ...
    return(sum(popRecon.log.prior.aug07(f, s, g, baseline.n, srb
...
popRecon.acc.ra.aug07 <- function(log.prop, log.current)
{
    min(1, exp(log.prop - log.current))
}

popRecon.acc.ra.var.aug07 <-
...
## if(!("ls.before" %in% ls.before)) ls.before <- c(ls.before, "ls.before")
## funs <- ls()[!(ls() %in% ls.before)]
## dput(funs, file = "")

funs.ch <- c("popRecon.acc.ra.aug07", "popRecon.acc.ra.var.aug07", "popRecon.dinvGamma.aug07",
             "popRecon.invlogit.aug07", "popRecon.logit.aug07",
             "popRecon.makeColNames.aug07",
             "popRecon.rinvGamma.aug07", "propRecon.log.lhood.aug07", "popRecon.log.post.aug07",
             "popRecon.proj.cen.yrs.aug07")

if(require(compiler))
{
    for(i in 1:length(funs.ch)) {
        assign(funs.ch[i], value = cmpfun(f = get(funs.ch[i])))
    }
}
...
popRecon.pop.est.sampler.aug07 <-
    function(## number of iterations and burn-in (not saved)
             ...
             ## (list with "female" and "male" components)
             ,pop.data

             ## rows of mean.f and start.f that are non-zero
             ,fert.rows = as.logical(apply(mean.f == 0L, 1, function(z) !all(z)))

             ## number of periods to project forward over (e.g., number of
             ## five-year steps)
             ...
             )
{
    ...
    ## Libraries
    if(filebacked.chains) require(ff)
    require(coda)
    require(mvtnorm)

    ## CCMP function
    ccmp.function <- match.fun(ccmp.f$FUN)
    if(!is.null(ccmp.f$formals)) {
        for(h in names(ccmp.f$formals)) formals(ccmp.function)[[h]] <- ccmp.f$formals[[h]]
    }
    ...
    if(filebacked.chains) {
        ...
        fert.rate.mcmc <-
            ff(dim = c(n.iter, sum(fert.rows) * ncol(start.f))
               ,dimnames = list(NULL
                ,popRecon.makeColNames.aug07(start.f[fert.rows,]))
               ,vmode = "double"
        ...
        surv.prop.mcmc <-
            list(female =
                 ff(dim = c(n.iter, nrow(start.s[["female"]]) * ncol(start.s[["female"]]))
                    ,dimnames = list(NULL
                     ,popRecon.makeColNames.aug07(start.s[["female"]]))
                    ,vmode = "double"
                 ...
                 ff(dim = c(n.iter, nrow(start.s[["male"]]) * ncol(start.s[["male"]]))
                    ,dimnames = list(NULL
                     ,popRecon.makeColNames.aug07(start.s[["male"]]))
                    ,vmode = "double"
                    ,filename = paste("surv_mcmc", file.suff, ".bin"
                     ,sep = "")
                    ,overwrite = TRUE
        ...
        lx.mcmc <-
            list(female =
                 ff(dim = c(n.iter, nrow(pop.data[["female"]]) * (proj.periods))
                    ,dimnames = list(NULL
                     ,popRecon.makeColNames.aug07(matrix(0, nrow = nrow(start.b[["female"]])
                                                         ,ncol = proj.periods
                 ...
                 ff(dim = c(n.iter, nrow(pop.data[["male"]]) * (proj.periods))
                    ,dimnames = list(NULL
                     ,popRecon.makeColNames.aug07(matrix(0, nrow = nrow(start.b[["male"]])
                                                         ,ncol = proj.periods
        ...
        mig.mcmc <-
            list(female =
                 ff(dim = c(n.iter, nrow(start.g[["female"]]) * ncol(start.g[["female"]]))
                    ,dimnames = list(NULL
                     ,popRecon.makeColNames.aug07(start.g[["female"]]))
                    ,vmode = "double"
                 ...
                 ff(dim = c(n.iter, nrow(start.g[["male"]]) * ncol(start.g[["male"]]))
                    ,dimnames = list(NULL
                     ,popRecon.makeColNames.aug07(start.g[["male"]]))
                    ,vmode = "double"
                    ,filename = paste("migProp_mcmc", file.suff, ".bin"
                     ,sep = "")
                    ,overwrite = TRUE
        ...
        baseline.count.mcmc <-
            list(female =
                 ff(dim = c(n.iter, nrow(start.b[["female"]]))
                    ,dimnames = list(NULL, popRecon.makeColNames.aug07(start.b[["female"]]))
                    ,vmode = "double"
                 ...
                 ff(dim = c(n.iter, nrow(start.b[["male"]]))
                    ,dimnames = list(NULL, popRecon.makeColNames.aug07(start.b[["male"]]))
                    ,vmode = "double"
                    ,filename = paste("baseline_mcmc", file.suff, ".bin"
                     ,sep = "")
                    ,overwrite = TRUE
        ...
        surv.prop.mcmc <-
            list(female = mcmc(matrix(nrow = n.iter
                 ,ncol = nrow(start.s[["female"]]) * ncol(start.s[["female"]])
                 ,dimnames = list(NULL
                  ,popRecon.makeColNames.aug07(start.s[["female"]])
                  )))
                 ,male = mcmc(matrix(nrow = n.iter
                  ,ncol = nrow(start.s[["female"]]) * ncol(start.s[["female"]])
                  ,dimnames = list(NULL
                   ,popRecon.makeColNames.aug07(start.s[["female"]])
                   )))
        ...
        ## baseline counts
        baseline.count.mcmc <-
            list(female = mcmc(matrix(nrow = n.iter, ncol = nrow(start.b[["female"]])
                 ,dimnames = list(NULL
                  ,popRecon.makeColNames.aug07(start.b[["female"]]))
                 ))
                 ,male = mcmc(matrix(nrow = n.iter, ncol = nrow(start.b[["male"]])
                  ,dimnames = list(NULL
                   ,popRecon.makeColNames.aug07(start.b[["male"]]))
                  ))
                 )

        ## srb
        srb.mcmc <-
            mcmc(matrix(nrow = n.iter, ncol = ncol(start.srb)
                        ,dimnames = list(NULL, colnames(start.srb))
                        ))

        ## variances
        variances.mcmc <-
            mcmc(matrix(nrow = n.iter, ncol = 5))
    ...
    logit.curr.s <-
        list(female = popRecon.logit.aug07(start.s[["female"]])
             ,male = popRecon.logit.aug07(start.s[["male"]])
             )
    ...
    logit.mean.s <-
        list(female = popRecon.logit.aug07(mean.s[["female"]])
             ,male = popRecon.logit.aug07(mean.s[["male"]])
             )
    ...
    log.curr.proj <-
        lapply(popRecon.proj.cen.yrs.aug07(full.proj =
                                           ccmp.function(pop = lapply(log.curr.b, "exp")
                                                         ,surv = lapply(logit.curr.s
                                                          ,"popRecon.invlogit.aug07")
                                                         ,fert = exp(log.curr.f)
                                                         ,mig = curr.g
    ...
    log.curr.posterior <-
        popRecon.log.post.aug07(f = log.curr.f
                                ,s = logit.curr.s
                                ,g = curr.g
                                ...
                                ,log.like = propRecon.log.lhood.aug07(
    ...
    for(i in 1:(n.iter + burn.in)) {
        ...
        for(j in 1:length(log.curr.f[fert.rows,])) {
            ...
            full.proj <- ccmp.function(pop = lapply(log.curr.b, "exp")
                                       ...
                                       ,surv = lapply(logit.curr.s, "popRecon.invlogit.aug07")
                                       ,mig = curr.g
                                       ,srb = exp(log.curr.srb)
                                       ,proj.steps = proj.periods
                                       ,age.int = age.size
            ...
            if(sum(full.proj.unlist < 0) > 0 || is.na(sum(full.proj.unlist)) ||
               is.nan(sum(full.proj.unlist))) {
                ...
                prop.proj <-
                    popRecon.proj.cen.yrs.aug07(full.proj = full.proj
                                                ,bline.yr = baseline.year
                                                ,vr.yrs = vr.years
                                                ,cen.yrs = census.years, proj.yrs = proj.years
                                                ,labels = FALSE
                ...
                log.prop.posterior <-
                    popRecon.log.post.aug07(f = log.prop.f #<-- use proposal
                                            ,s = logit.curr.s
                                            ,g = curr.g
                                            ,baseline.n = log.curr.b
                                            ,srb = log.curr.srb
                ...
                ## Acceptance ratio
                ar <- popRecon.acc.ra.aug07(log.prop = log.prop.posterior,
        ...
        for(j in 1:length(logit.curr.s[["female"]])) {
            ...
            logit.fm.prop <- rmvnorm(1, sigma = prop.varcovar[["surv.prop"]][[j]])
                                        # first is female
            ...
            if(popRecon.invlogit.aug07(logit.prop.s.unlist[j]) > 1 - s.tol ||
               popRecon.invlogit.aug07(logit.prop.s.unlist[j]) < s.tol) {
                ...
                full.proj <-
                    ccmp.function(pop = lapply(log.curr.b, "exp")
                                  ,fert = exp(log.curr.f)
                                  ,surv = lapply(logit.prop.s, "popRecon.invlogit.aug07") #<-- use prop
                                  ,srb = exp(log.curr.srb)
                                  ,mig = curr.g
                                  ,proj.steps = proj.periods
                                  ,age.int = age.size)
        ...
        if(i > burn.in) for(l in 1:2)
            surv.prop.mcmc[[l]][k,] <- as.vector(popRecon.invlogit.aug07(logit.curr.s[[l]]))
        ...
        for(j in 1:length(curr.g[["female"]])) {
            ...
            fm.prop <- rmvnorm(1, sigma = prop.varcovar[["mig"]][[j]])
                                        # first is female
        ...
        for(j in 1:length(log.curr.b[["female"]])) {
            ...
            fm.prop <- rmvnorm(1, sigma = prop.varcovar[["baseline.pop.count"]][[j]])
                                        # first is female
        ...
        if(v.prop.type == "inverse.gamma") {
            prop.sigmasq.f <-
                popRecon.rinvGamma.aug07(1, al.f +
        ...
        ## Acceptance ratio
        if(v.prop.type == "inverse.gamma") {
            ar <- popRecon.acc.ra.var.aug07(log.prop.post = log.prop.posterior
                                            ,log.curr.post = log.curr.posterior
                                            ,log.prop.var = popRecon.dinvGamma.aug07(prop.sigmasq.f
                                             ,al.f + length(mean.f[fert.rows,])/2
                                             ,be.f + 0.5*sum((log.curr.f[fert.rows,] -
                                                              log.mean.f[fert.rows,])^2)
                                             ,log = TRUE)
                                            ,log.curr.var = popRecon.dinvGamma.aug07(curr.sigmasq.f
                                             ,al.f + length(mean.f[fert.rows,])/2
                                             ,be.f + 0.5*sum((log.curr.f[fert.rows,] -
                                                              log.mean.f[fert.rows,])^2)
                                             ,log = TRUE)
            ...
            ar <-
                popRecon.acc.ra.var.aug07(log.prop.post = log.prop.posterior
                                          ,log.curr.post = log.curr.posterior
        ...
        if(v.prop.type == "inverse.gamma") {
            ...
            prop.sigmasq.s <-
                popRecon.rinvGamma.aug07(1, al.s + hlf.l.mean,
        ...
        if(v.prop.type == "inverse.gamma") {
            ar <- popRecon.acc.ra.var.aug07(log.prop.post = log.prop.posterior
                                            ,log.curr.post = log.curr.posterior
                                            ,log.prop.var = popRecon.dinvGamma.aug07(prop.sigmasq.s
                                             ,al.s + hlf.l.mean
                                             ,be.s + hlf.sum.diff.sqrd
                                             ,log = TRUE)
                                            ,log.curr.var = popRecon.dinvGamma.aug07(curr.sigmasq.s
                                             ,al.s + hlf.l.mean
                                             ,be.s + hlf.sum.diff.sqrd
                                             ,log = TRUE)
                                            )
        ...
        if(v.prop.type == "inverse.gamma") {
            ...
            ## Proposal
            prop.sigmasq.g <-
                popRecon.rinvGamma.aug07(1, al.g + hlf.l.mean,
        ...
        if(v.prop.type == "inverse.gamma") {
            ...
            ar <- popRecon.acc.ra.var.aug07(log.prop.post = log.prop.posterior
                                            ,log.curr.post = log.curr.posterior
                                            ,log.prop.var = popRecon.dinvGamma.aug07(prop.sigmasq.g
                                             ,al.g + hlf.l.mean
                                             ,be.g + hlf.sum.diff.sqrd
                                             ,log = TRUE)
                                            ,log.curr.var = popRecon.dinvGamma.aug07(curr.sigmasq.g
                                             ,al.g + hlf.l.mean
                                             ,be.g + hlf.sum.diff.sqrd
                                             ,log = TRUE)
                                            )
        ...
        if(v.prop.type == "inverse.gamma") {
            ...
            ## Proposal
            prop.sigmasq.n <-
                popRecon.rinvGamma.aug07(1, al.n + hlf.l.mean / 2,
        ...
        if(v.prop.type == "inverse.gamma") {
            ...
            ar <- popRecon.acc.ra.var.aug07(log.prop.post = log.prop.posterior
                                            ,log.curr.post = log.curr.posterior
                                            ,log.prop.var = popRecon.dinvGamma.aug07(prop.sigmasq.n
                                             ,al.n + hlf.l.mean
                                             ,be.n + hlf.sum.diff.sqrd
                                             ,log = TRUE)
                                            ,log.curr.var = popRecon.dinvGamma.aug07(curr.sigmasq.n
                                             ,al.n + hlf.l.mean
                                             ,be.n + hlf.sum.diff.sqrd
                                             ,log = TRUE)
                                            )
...
## ## Use Sri Lanka. Just make males same as females for now.
...
##     Fal.root <- "T:/"
##     Caen.root <- "H:/"
##     Caen.MD <- paste(Caen.root, "Profile/My Documents", sep = "")
## } else if(Sys.info()[["sysname"]] == "Linux" &
##           substr(Sys.info()[["nodename"]], 1, 3) == "fal") {
##     Fal.root <- "~/"
## } else {
##     Caen.MD <- "~/Documents/Caen.MD/"
##     Fal.root <- "~/Documents/Faloorum/"
##     }

## data.path <- paste(Fal.root, "PPGp/SrLaF/DataSets/", sep = "")
## prog.path <- paste(Fal.root, "PPGp/RPrograms/", sep = "")

## source(paste(Fal.root, "PPGp/RPrograms/Misc_Functions.R", sep = ""))
## source(file.path(prog.path
##              ,"CCMPProjection/ccmp_femDom_20120402.R"))
## source(file.path(prog.path
##                  ,"ResultsSummary/CondVariances_20110208.R"))
## source(file.path(prog.path
##                  ,"Hyperparameters", "ChooseHyperparameters_20120126.R"))

## save.output.path <-
##     file.path(Fal.root, "PPGp/RPrograms/EstimationModel/TEST_2sex_20120330")

## setwd(save.output.path)
...
## load(file.path(data.path, "SLFem_ASFR_SmTFRPatnMeds_20111006LOCKED.Rdata"))
## load(file.path(data.path, "SLFem_ASSURV_20110830LOCKED.Rdata"))
## load(file.path(data.path, "SLFem_CensusCounts_20110919LOCKED.RData"))


## ## Only use upto and including 2001. Last census is at 2001 so the
## ## inputs for 2001 AND 2006 will not be updated.

## asFertSLFEM.mat <-
##     asFertSLFEM.mat[, as.character(seq(from = 1951, to = 2001, by = 5))]
## asSurvSLFEM.mat <-
##     asSurvSLFEM.mat[, as.character(seq(from = 1951, to = 2001, by = 5))]


## ## Make migration

## asMigSLFEM.mat <- matrix(0, ncol = ncol(asFertSLFEM.mat)
##                          ,nrow = nrow(asFertSLFEM.mat)
##                          ,dimnames = dimnames(asFertSLFEM.mat)
##                          )
...
## load(file = file.path(Fal.root
##      ,"PPGp/SrLaF/EstMod_RUNS/SrLaF_20120126/vitalCondVars_0126_A.Rdata"))

## prop.vars <- lapply(vitalCondVars, "*", 2.3^2)
## names(prop.vars) <-
##     c("fert.rate", "surv.prop", "mig", "population.count")

## prop.vars[["mig"]] <- rep(0.5, length(prop.vars[["mig"]]))
## prop.vars[["population.count"]] <- rep(0.5, length(prop.vars[["population.count"]]))

## prop.corrs <- prop.vars
## for(i in 1:length(prop.corrs)) {
##     for(j in 1:length(prop.corrs[[i]])) {
##         prop.corrs[[i]][j] <- 0.9
##     }}

## prop.varcovar <- prop.vars
## for(i in 1:length(prop.varcovar)) {
##     prop.varcovar[[i]] <-
##         replicate(length(prop.vars[[i]]), matrix(rep(0, 4), ncol = 2), simplify = FALSE)
##     for(j in 1:length(prop.varcovar[[i]])) {
##         v <- prop.vars[[i]][j]
##         cv <- v * prop.corrs[[i]][j]
##         prop.varcovar[[i]][[j]] <- cbind(c(v, cv), c(cv, v))
##     }
## }
...
##             ,ccmp.f = list(FUN = "ccmp.femDom.apr02")
##             ,start.f = asFertSLFEM.mat
##             ,start.s = list(female = asSurvSLFEM.mat, male = asSurvSLFEM.mat)
##             ,start.g = list(female = asMigSLFEM.mat, male = asMigSLFEM.mat)
##             ,start.b = list(female = baselineSLFEM.mat, male = baselineSLFEM.mat)
##             ,start.sigmasq.f = 1
##             ,start.sigmasq.s = 1
##             ,start.sigmasq.g =1
##             ,start.sigmasq.n =1
##          ,start.srb = 1.05
##             ,mean.f = asFertSLFEM.mat
##             ,mean.s = list(female = asSurvSLFEM.mat, male = asSurvSLFEM.mat)
##             ,mean.g = list(female = asMigSLFEM.mat, male = asMigSLFEM.mat)
##             ,mean.b = list(female = baselineSLFEM.mat, male = baselineSLFEM.mat)
##          ,srb.lims = c(1.05, 1.05)
##              ,fert.rows = as.logical(apply(asFertSLFEM.mat == 0L, 1, function(z) !all(z)))
##             ,prop.varcovar = prop.varcovar
##             ,pop.data = list(female = censusSLFEM.mat, male = censusSLFEM.mat)
##             ,proj.periods = ncol(asFertSLFEM.mat)
##             ,age.size = 5
##             ,s.tol = 10^(-10)
##             ,verb = TRUE, progress.step = 1
##             ,filebacked.chains = FALSE
##             ,file.suff = ""
##          ,v.prop.type = "inverse.gamma"
##              ,prop.varcovar$variances = list(fert.rate = 1, surv.prop = 1
##               ,mig = 1, pop.count = 1)
##          ,debug.objs = TRUE
##              )

## for(i in 1:length(estModArgs.0330)) {
##     assign(names(estModArgs.0330)[i], estModArgs.0330[[i]])
## }
...
## ## SLFemMcmc.0330 <-
## ##     do.call(popRecon.pop.est.sample.aug07, args = estModArgs.0330)

## ## save(SLFemMcmc.0330
## ##      ,file = file.path(save.output.path,
## ##       "SLFem_OUTPUT_20120330.Rdata"
## ##       )
## ##      )
...
## plot(log.post.mcmc[1:600,1], type = "l")
## points(log.post.mcmc[1:600,2], col = "red")

## abline(v = 1 + fert.J, lty = 3, col = "grey")
## abline(v = 1 + fert.J + surv.J, lty = 3, col = "grey")
## abline(v = 1 + fert.J + surv.J + mig.J, lty = 3, col = "grey")
## abline(v = 1 + fert.J + surv.J + mig.J + baseline.J, lty = 3, col = "grey")
