...
## Since 2012-08-08
## - Now gives 'message' instead of 'warning' when objects placed in global env.

## Since 2012-04-07
## - More notes and comments. No change to code.
...
proj.to.counts.aug22 <-
    function(fert.rate.mcmc, surv.prop.mcmc, mig.prop.mcmc
             ,baseline.count.mcmc, pop.count.mcmc
             ,srb.mcmc =1.05
             ,sep.factors = list(female = 0.5, male = 0.5)
             ,ccmp.f
             ,outputs = c("birth.count", "total.birth.count"
              ,"total.birth.death.count"
              ,"death.count", "mig.count", "total.mig.count"
              ,"mort.rate", "mig.rate", "person.years", "cohort.nq0", "period.nq0", "IMR")
             ...
             )
{
    ## -------------------------------------------------------------------------
    ##
    ## PUROPSE:         Converts the quantities used in population projection
    ##                  (fert rates suvival proportions, migration proportions,
    ##                  counts) into counts of births, deaths and net number of
    ##                  migrants.
    ##
    ##
    ## ARUGMENTS:
    ##
    ## fert.rate.mcmc, ..., baseline.count.mcmc:
    ##                  Lists of mcmc objects with two components named 'female'
    ##                  and 'male'. Column names are yyyy.aa.aa
    ##                  (year.age-start.age-end).
    ##
    ##                  mig.prop.mcmc is assumed to be the average
    ##                  _annual_ proportion. It is multiplied by
    ##                  age.int to get migration for the entire interval.
    ##
    ##                  pop.count.mcmc does not contain baseline.count.mcmc.
    ##
    ##                  These should all have the same number of rows. The
    ##                  output will have this many rows too, otherwise the same
    ##                  number of rows as the smallest of these inputs.
    ##
    ## SRB:             Sex ratio at birth. Single numeric vector, length 1, or
    ##                  an mcmc/matrix with
    ##
    ## sep.factors:     List of separation factors used to separate cohort deaths into
    ##                  period deaths with components names 'female' and
    ##                  'male. These can be a scalar, in which case the same
    ##                  factor is used for all ages, year, or a _vector_ with
    ##                  names yyyy.aa indicating the 'y'ear and 'a'ge. Age goes
    ##                  from 0 to the open ended interval in the population
    ##                  counts (not the survival proportions).
    ##
    ##                  For sep factor k_a, {delta}_D_a^c = k_a * D_a^c, i.e.,
    ##                  k_a is the fraction of cohort deaths for cohort aged 'a'
    ##                  that occurs in the first triangle of the cohort
    ##                  parallelogram.
    ##
    ##                  sep factor for oldest age group is currently re-set to 1.
    ##
    ## ccmp.f:          CCMPP function found by 'match.fun(ccmp.f)'
    ##
    ## outputs:         Character vector listing the counts required as output.
    ##                  Counts are calculated regardless but are not output
    ##                  unless requested.
    ##
    ...
    ## OUTPUTS:
    ##
    ##           : The requested outputs are matrices and are placed
   ...
   require(coda)
    ...
    if(!identical(head(unique(surv.prop.ages), -1)
                  ,unique(mig.prop.ages)) ||
       !identical(unique(mig.prop.ages)
    ...
    ## Can be calculated swiftly using matrix operations
...
## load(file = file.path(Fal.root, "PPGp/Laos/EstMod_RUNS/Laos_20120611"
##       ,"Laos_OUTPUT_20120611.Rdata"
##       )
##      )

## load(file = file.path(Fal.root, "PPGp", "Laos", "DataSets"
##       ,"LaFem_CDWest_sep_facs_20120402.Rdata")
##      )
## load(file = file.path(Fal.root, "PPGp", "Laos", "DataSets"
##       ,"LaMal_CDWest_sep_facs_20120402.Rdata")
##      )


## ### Assign arguments to global env
## ###

## fert.rate.mcmc <- LaosMcmc.0611$fert.rate.mcmc
## surv.prop.mcmc <- list(female = LaosMcmc.0611$surv.prop.mcmc[["female"]]
##                        ,male = LaosMcmc.0611$surv.prop.mcmc[["male"]])
## mig.prop.mcmc <- list(female = LaosMcmc.0611$mig.prop.mcmc[["female"]]
##                       ,male = LaosMcmc.0611$mig.prop.mcmc[["male"]])
## baseline.count.mcmc <- list(female = LaosMcmc.0611$baseline.count.mcmc[["female"]]
##                             ,male = LaosMcmc.0611$baseline.count.mcmc[["male"]])
## pop.count.mcmc <- list(female = LaosMcmc.0611$lx.mcmc[["female"]]
##                        ,male = LaosMcmc.0611$lx.mcmc[["male"]])
## srb.mcmc <- LaosMcmc.0611$srb.mcmc
## outputs <- c("birth.count", "total.birth.count"
##               ,"total.birth.death.count"
##               ,"death.count", "mig.count", "total.mig.count"
##               ,"mort.rate", "mig.rate", "person.years", "cohort.nq0", "period.nq0", "IMR")
...
## file.backed <- TRUE
## sep.factors <- list(female = CDW.lafem.sf[1:68]
##                       ,male = CDW.lamal.sf[1:68])
